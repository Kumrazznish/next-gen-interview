"use client";
  ...
  const Chatbot: React.FC = () => {
    ...
    const handleGenerateQuestions = async () => {
      if (!selectedRole || !selectedJobType || !selectedExperience || !skills) return;
  
      setLoading(true);
      setMessages((prev) => [...prev, { text: `Generating ${numQuestions} question(s)...`, isBot: true }]);
      ...
      try {
        const response = await fetch(`${GEMINI_API_URL}?key=${GEMINI_API_KEY}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `Generate ${numQuestions} interview questions for ${selectedJobType} (${selectedExperience}) requiring ${skills}. 
                Format each question starting with 'Q1.', 'Q2.', etc. Return only the questions without any additional text.`
              }]
            }]
          }),
        });
        ...
        try {
          const response = await fetch(`${GEMINI_API_URL}?key=${GEMINI_API_KEY}`, {
            ...
            body: JSON.stringify({
              contents: [{
                parts: [{
                  text: `Provide a concise answer to this question: ${message.text}`
                }]
              }]
            }),
          });
    ...
    const chatWindowClasses = `p-4 h-[500px] overflow-y-auto rounded-lg ${
    ...
    return (
      <div className={containerClasses}>
        ...
        <div className={leftPanelClasses}>
          <h1 className="text-2xl font-bold mb-4">🤖 Interview Bot</h1>
